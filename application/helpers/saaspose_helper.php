...
/**
	...
	function Sign($UrlToSign, $AppSID, $AppKey) {
        // parse the url
        $url = parse_url($UrlToSign);
      
        if (isset($url['query']) == "")
            $urlPartToSign = $url['path'] . "?appSID=" .  $AppSID;
        else
            $urlPartToSign = $url['path'] . "?" . $url["query"] . "&appSID=" . $AppSID;
 
        // Decode the private key into its binary format
        $decodedKey = decodeBase64UrlSafe($AppKey);
        ...
        if (isset($url['query']) == "")
            return $url["scheme"] . "://" . $url["host"] . $url["path"] . "?appSID=" . $AppSID . "&signature=" . $encodedSignature;
        else
            return $url["scheme"] . "://" . $url["host"] . $url["path"] . "?" . $url["query"] . "&appSID=" . $AppSID . "&signature=" . $encodedSignature;
    }
